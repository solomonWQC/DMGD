...
from SemanticMatching import random_label_conditioning
...
class DiT(nn.Module):
    ...
    def forward(self, x, t, y, random_label=0, t_upper=900, t_lower=500, noise_weight=0.06, label_weight=0.01):
        """
        Forward pass of DiT.
        ...
        random_y = self.y_embedder(random_label, self.training)
        y = random_label_conditioning(t_ori, y, random_y, t_upper=t_upper, t_lower=t_lower,
                                      noise_weight=noise_weight, label_weight=label_weight)
        c = t + y                                # (N, D)
        for block in self.blocks:
            x = block(x, c)                      # (N, T, D)
    ...
    def forward_with_cfg(self, x, t, y, cfg_scale, **kwargs):
        ...
        model_out = self.forward(
            combined, t, y,
            random_label=kwargs['random_label'],
            t_upper=kwargs['t_upper'],
            t_lower=kwargs['t_lower'],
            noise_weight=kwargs['noise_weight'],
            label_weight=kwargs['label_weight']
        )
