...
def save_accuracies_to_file(accuracies, folder_path="results", filename="accuracies.txt"):
    """将准确率列表保存到指定文件夹的文本文件中
    
    参数:
    accuracies (list): 包含准确率值的列表
    folder_path (str): 保存文件的文件夹路径，默认为"results"
    filename (str): 保存的文件名，默认为"accuracies.txt"
    """
    try:
        # 确保文件夹存在
        ...
        print(f"成功将 {len(accuracies)} 个准确率值保存到 {file_path}")
...
if __name__ == '__main__':
    ...
    name =args.net_type+str(args.ipc)+'.txt'
    acc=main(args, logger,5)
    save_accuracies_to_file(acc,args.imagenet_dir[0],name)
    #save_accuracies_to_file(std,args.imagenet_dir[0])
